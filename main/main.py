...
class Author(db.Model):
    ...
    books = db.relationship("Author_Book")
...
class Book(db.Model):
    ...
    authors = db.relationship("Author_Book")
...
query = db.session.query(Book.title, Genre.name, Author.name).join(Author_Book).join(Genre).join(Author)
# query_1 = db.session.query(Author.name).join(Author_Book)
